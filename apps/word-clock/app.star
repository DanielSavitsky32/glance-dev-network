...
PAL = {"ink": "#F4F7FF", "ghost": "#2A3140", "accent": "#E8B04B"}

# ---- time zone -------------------------------------------------------------
# ctx.now is UTC. The zone is a dropdown resolved locally -- no network, and no
# zip lookup that has to render a different image for every zip code when the
# clock face only ever differs by offset. One entry per distinct wall clock:
# Paris, Berlin, Madrid and Rome are the same picture, so they are one row.
#
# name -> [standard offset in minutes east of UTC, daylight-saving rule]
#   0: none   1: US/Canada (2nd Sun Mar -> 1st Sun Nov)   2: EU (last Sun Mar
#   -> last Sun Oct, 01:00 UTC). Same rules as apps/google-calendar.
ZONES = {
    "HAWAII": [-600, 0],
    "ALASKA": [-540, 1],
    "PACIFIC": [-480, 1],
    ...
    "LONDON": [0, 2],
    "CENTRAL EUROPE": [60, 2],
    "EASTERN EUROPE": [120, 2],
    "MOSCOW": [180, 0],
    "DUBAI": [240, 0],
    "INDIA": [330, 0],
    "CHINA / SINGAPORE": [480, 0],
    "JAPAN / KOREA": [540, 0],
}
...
def zone_offset_at(zone, t):
    ...
    if rule == 1:
        # 2nd Sunday in March at 02:00 standard -> 1st Sunday in November at
        # 02:00 daylight, which is 01:00 standard.
