...
def eastern_offset_seconds(ctx):
    return zone_offset_at("America/New_York", ctx.now.unix // 60) * 60
...
def local_race_date(ctx, race_date_str):
    ...
    et_off = eastern_offset_seconds(ctx)
...
def draw_live_intro(c, state, ctx):
    ...
    if is_race:
        ...
        if is_f1:
            flag_str = state["flag"]
            c.text(flag_str, sx, 2, font = "5x7", color = F1_FLAG_COLOR.get(flag_str, COLORS["muted"]))
            c.text("LAP " + str(state["lap"]), sx, 12, font = "4x5", color = COLORS["text"])
            weather_txt = state.get("weather", "")
            if weather_txt != "":
                c.text(weather_txt, sx, 21, font = "4x5", color = COLORS["accent2"])
        else:
            c.text(FLAG_LABEL.get(state["flag"], "FLAG"), sx, 2, font = "5x7", color = flag_color(state["flag"]))
            c.text("LAP " + str(state["lap"]) + "/" + str(state["laps_total"]), sx, 12, font = "4x5", color = COLORS["text"])
            stage_txt = "STAGE " + str(state["stage_num"]) + "/3" if state["stage_num"] > 0 else "STAGE -"
            c.text(stage_txt, sx, 21, font = "4x5", color = COLORS["accent2"])
